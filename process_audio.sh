...
# Usage: ./normalize_audio.sh input.wav output.wav [target_lufs]

# Check arguments
if [ $# -lt 2 ]; then
    echo "Usage: $0 <input_file> <output_file> [target_lufs]"
    echo "Example: $0 input.wav output.wav -19"
    exit 1
fi
...
# Run FFmpeg and capture all stderr output
ffmpeg -i "$INPUT_FILE" -af acompressor=threshold=-21dB:ratio=3:attack=200:release=1000,loudnorm=I=$TARGET_LUFS:TP=-1.5:LRA=11:print_format=json -f null - 2> "$TEMP_OUTPUT"
...
echo "Debug: FFmpeg output saved to $TEMP_OUTPUT"
echo "Debug: JSON extraction attempt saved to $TEMP_JSON"
